...
**Status: IN-PROGRESS** — M2.1-M2.5 DONE; M2.6 and M2.6b both closed negative and ruled. **M2.7 and M2.8 are PLANNED into 8 units** — M2.7 DONE (M2.7.1-M2.7.4); M2.8.1 DONE; **M2.8.2 DONE across two windows** (tracker-freeze defect fixed, then the driver, the 379-row manifest and the full corpus run at 8.70 h measured); **M2.8.4 DONE across four windows** (isotropic coordinates plus the `--tracking body` corpus re-run, 193/193 at 7.828 h, the funded orientation fix refuted before it was built); **M2.8.3 DONE across three**, then split at its `FEATURES` seam by user ruling; **M2.8.5 DONE in two** — the cohort publisher ships and publishes over `output/corpus-2d`. **Every unit is DONE, so M2 is IMPLEMENTED and the next session runs MILESTONE-REVIEW.** M2.7 publishes the negative through a new `calibration_qc/` publisher (F1a); **M2.8 publishes cohort aggregates only, by user ruling** — no per-subject rows, no patient identifier, no join column, no join to `../rehab`, and it stops at this repo's boundary. The 3D line is closed; it reopens only on prospective calibrated capture. The old clearance precondition is met: full decode clearance covers the whole `videos/3-cam/` tree, for MAIN and teammates. Chat and reports carry redacted aggregates only — never imagery, filenames, or subject identifiers.
...
`tests/` and **10/10 green**, so all 27 restored reds now ride the decisive gate and nothing sits in
`.scratch/review-reds/`. Shipped: the pilot validates the inventory and qualification generations
before a cell of either becomes a stratum label or an allowlist string (R14); `_run_event` clears
its event tree before launch (R15); the driver's report is a function of STATE rather than of the
pass — `events_complete`/`events_failed`/`events_unattempted` from the markers, `invocation_wall_s`
deleted, invocation counters to stdout (R20); `process_source` **re-raises** `KeyboardInterrupt`
(R21); `_artifacts` checks the diagnostics row's own `video` ownership key, **measured 379/379 on the
shipped corpus before shipping** (R25); `multicam.published_overlap` is extracted public and both
scripts refuse any sink overlapping the published tree, before the first mkdir and again before the
report write (R27); the NaN-blind rate comparison is inverted (R28); and the redaction guard's shape
backstop is **deleted** for membership in a frozen `REPORT_FIELDS`, with the suite's oracle now
delegating to the shipped `redaction_violations` rather than re-implementing it (R29/R32).
**Two data.** One reviewer's own two reds contradicted each other — R20's fixture omitted the `video`
column R25's fix requires, so applying both left R20 red on the wrong predicate; reds from one
reviewer are no more mutually consistent than reds from two, so restore the battery and run it as a
set. And the R21 fix **halted a whole pytest session**: the one shipped case asserting the swallow
propagated its `KeyboardInterrupt` into the runner, which is what a behaviour change from return to
raise costs. Editing `multicam.py` also fired the calibration-QC source-digest tripwire; both it and
the qualify campaign refuse to regenerate over a stale result by design, so the two JSON files were
removed and both campaigns rerun **last**, green at 21 sweeps / 18 tampers / 39 PASS / 0 FAIL and
40 sweeps / 40 passed / 19 tamper classes / 0 tamper failures.

**MILESTONE-REVIEW wave 2, session 1 — fix application; M2 stays IMPLEMENTED.** MAIN implemented every fix itself: the authorship split makes kernel code MAIN-authored and every open row is kernel or spec surface, which matches six consecutive windows of near-zero map-delegation value. **23 of the 43 ACCEPT-FIX rows are closed, 20 remain** (T1 0/10 · T2 3/7 · T3 14/14 · T4 2/4 · T5 3/7 · T6 1/1). The three reviewer tips carry the acceptance checks in executable form — restored with `git show archive/m2-review-<name>:tests/<file>`, **27 red at restore**, 17 green now; `tests/test_review_m2u82.py` stays parked at `.scratch/review-reds/` with its 10 reds, since `test_c8_08` runs the whole suite in a subprocess and one red file under `tests/` takes the decisive gate down. Closed here: the **isotropy cluster** (T2 R24 = T3 R05 = T3 R39 = T4 R41 — `cohort.UNIT_DEG` → `deg_image_plane`, `docs/technical/cohort.md` boundary rewritten, and `cohort/` **republished on the real corpus: 12 cells, 1068 feature rows, 89 published / 3 excluded, reconciling exactly with the recorded census**, `descriptors.yaml` 19 `deg_image_plane` / 0 `deg_image_plane_uncalibrated`), the **cohort publisher's atomicity rewrite** (A30: `tempfile.mkdtemp` staging, `_sweep_orphans` deleted rather than taught an ownership marker — a prefix-name sweep deleted foreign trees *and* complete generations), five further cohort predicates (A26-A29), and the determinism evidence for all three campaigns. **Datum: a restored red is evidence, not law** — 2 of the 27 were themselves defective (the M2.8.2 A10 class), one contradicting its own ledger row with the *test* right. Also closed: the roadmap/contract figure row (T2 R28 = T5 R23) and the project-law scale correction (T5 R25), both discharged by correcting the frozen records in place — `contract-m2u83.md` gains **A25-A30** and A09's superseded token now carries pointers at §5, at A09 and at A20. Remaining 20: T1's 10 corpus-run rows behind the parked battery, T2's 4, T4's 2, T5's 4 including the publisher trust-root triple. **Decisive gate alone from the final state: 1720 passed in 1307.40 s (21:47), rc=0** — 1702 plus the 18 restored review cases.
...
**CLOSED at MILESTONE-REVIEW wave 1.** `audit-m2` R16-R19 re-searched all four absences (standards
bodies, vendor guidance, MHLW/MEXT indexes, the OpenCap/Pose2Sim/Anipose primaries) and **all four
pass**: task-specific reprojection targets exist but no universal clinical cutoff; board sizing is
ruled in image space and volume coverage, never as a physical board:volume ratio; Japan's 5-year /
3-year minimums are scoped to invasive interventional research, leaving non-invasive video with no
generic term; the three toolchains publish frame-lag methods with no validated millisecond
acceptance figure. Evidence per row in `.agent/archive/review-m2-audit-m2.md`. **V17-V20 are
`confirmed`, so M2.7.4's verification is 20/20 and this coverage limit no longer stands.**
...
in force then. The always-resident set is now 243 KB — `roadmap.md` + `polish.md` attached (215 KB)
plus the bare `.claude/rules/` tier (28 KB), which every teammate inherits too — so entry cost stands
at **~128K**, with 83 KB of area-bound rules lazy behind `paths:`. MAIN's window is the 273K
...
### M2.8 — 2D cohort aggregate delivery — PLANNED, 4 units
...
| M2.8.3 | kernel | **Published schema half.** `cohort.FEATURES` = 89 features × (`level`, `column`, `ja`, `en`, `unit`, `range`), composed from 20 family pairs × side × derivation; the closed 7-token unit vocabulary; `CohortError`; `describe`. | 2 est; **3 spent, DONE at `4c006bd`** — 22 amendments, counts measured (89 published / 1068 rows / 12 cells), A02 cross-check green, static gate rc=0 |
| M2.8.4 | data + kernel fix | **Corrected corpus re-run (~8.7 h), carrying two repairs.** (a) `--tracking body`, populating the trunk/posture columns — **measured 17 present, 14 finite, 3 sagittal `NA`, 89 published** (the plan predicted 17/92; §M2.8.3 window 3 carries the measurement); (b) **isotropic coordinates** — one scalar `max(frame_w, frame_h)` at `export.py`'s three normalisation sites, killing the 9.9° angle distortion and the 3.16× portrait/landscape contamination. **The planned orientation fix is REFUTED and must not be built** — see below. One run carries both. **Unblocks M2.8.3.** | 1-2 est; **4 spent, DONE** — 17/17 predicates pass, corpus 193/193 at 7.828 h, `output/corpus-2d` swapped |
| M2.8.5 | kernel | **Compute + publication half, split out of M2.8.3.** Four-stage subject estimand (D02/A03) over the 12 cells × 89 features; `view_dispersion` (A06) over the 135 multi-asset events; staged publication + swap with the A13/A14 marker; `validate_generation` (P17/A16); `cohort.json` census carrying `rows_zero_values` / `rows_without_multiview` / `rows_below_subject_floor` / `descriptor_collision` (A04/A07/A10); `descriptors.yaml` projection (A08); CLI `main --inventory --sessions --run --out` (A17); `scripts/check_cohort_determinism.py` (A17); `docs/technical/cohort.md` (P18); registration in four doc indexes + `pyproject.toml` 10 → 11 + `.gitignore`. | **2-3 est, DONE in 2** — inherits M2.8.3's contract, its 24 amendments and its red suite; window 1 bought the design, window 2 shipped every dispatch item |
...
#### M2.8.5 — DONE, 2 windows against a `2-3 est`. The cohort publisher ships

**Window 1 bought the design and shipped no code; window 2 shipped every one of the ten dispatch
items.** That split is the unit's sizing datum: **on a unit whose specification is a 2169-line
diff-blind suite, the suite read is the window's dominant cost and must be budgeted as its own
window.** Window 1's entry cost — attached state, the contract with 22 amendments, the suite,
`cohort.py` and the sibling's publication machinery — consumed the whole window and produced one
artifact: a complete implementation design plus the A23 ruling, carried forward in the compaction
summary. Window 2 implemented from that summary rather than from a re-read and closed in one pass.
**PLANNING should treat a large diff-blind suite as a read-window plus an implementation window, not
as one unit of work.**

**Shipped.** `src/pose_estimation/cohort.py` gains the compute + publication half beside the schema
half: the four-stage estimand (D02/A03), `view_dispersion` (A06), staged publication with ownership,
swap and orphan sweeping (A13/A14/A24), `validate_generation` (P17/A16), the `cohort.json` census
(A04/A07/A10/A22), the `descriptors.yaml` projection rendered without a YAML emitter for byte
control (A08/A15), and the CLI (A17). Plus `scripts/check_cohort_determinism.py`,
`docs/technical/cohort.md`, and registration in four doc indexes, `pyproject.toml` (10 → 11
commands) and `.gitignore`.

**Measured on the real corpus** — `pose-estimation-cohort --inventory inventory --sessions sessions
--run output/corpus-2d --out cohort`, **14.4 s**, 180 KB published. Every number reconciles with the
census M2.8.3 measured independently: **12 cells, 1068 feature rows, 89 published / 3 excluded
columns, 379 assets, 193 events, 16 subjects, 331 152 frame rows, 21 483 window rows**, `n_subjects`
15-16 per cell, `descriptor_collision` = `{checked: true, n_external: 219, n_collisions: 0}`.
Independent check on `view_dispersion`: per-feature multi-view event totals run **130-135 against
the 135 multi-asset events**, so A06's population bound is reached exactly and the shortfall is only
where one camera carries no finite value for that feature. `rows_zero_values`,
`rows_without_multiview` and `rows_below_subject_floor` are all **0** on real input — those three
census paths are reachable synthetically alone, as M2.8.3 predicted for P08.

**Gates.** Red suite **60/60 green**; full decisive gate **1702 passed** with `ruff check` +
`ruff format --check` + `ty check` all rc=0. `scripts/check_cohort_determinism.py` reads **PASS — 6
publication sweeps, 15 consumer-boundary tamper classes, 1.2 s**; it is the published tree's byte
oracle, because `cohort/` is gitignored and gets no committed golden. Sweeps: republish over self,
different output name, moved cwd, moved pid, rebuilt corpus. Its `source_digest` covers both
`cohort.py` and `tests/test_cohort.py`, so a suite edit invalidates a recorded PASS rather than
letting it certify bytes it never produced.

**Three rulings, all recorded in the contract.** **A22 DISCHARGED** — the engine fixed the four
deferred domains and the suite now pins them exactly (`_POPULATION_KEYS`, `_ESTIMAND`,
`_COLLISION_SOURCE`, `_INPUT_DIGEST_KEYS`), in this same commit as the obligation required.
**A23** — A10's five-subject floor and P08's oracle contradicted each other on three cells; the
floor won and the test's expectation now carries the floor. **A24** — A13's digest rule left the
census uncovered while P17 edits it, so `tree_digest` covers the three files plus the whole marker
body minus its own self-referential key.

**Glyph delta MEASURED, item 5 closed.** The 89 `ja` labels use 75 non-ASCII characters; all four
`ibm-plex-sans-jp-*.woff2` faces in `../rehab` are identically subset and each misses the same
**14** — `チペ偏勢姿尖幹扇把斜旋称肘隔`. The six Latin faces carry no CJK. The published bytes are
correct; the consumer's font subset is short. The fix is in `../rehab`, outside this repo's
boundary, so it ships as a `.agent/polish.md` entry with the glyph list and its acceptance check.

**Window 2 gauges — `harvest=` n/a (MAIN-only unit, no wave), `main=` 53% 144K/273K, `mate=` n/a.**
Window 1: `main=` ~90% at compaction, 0 dispatch items shipped. The contrast is the datum: the same
unit cost a full window to read and half a window to build.

**Gate recipe — `env -u LD_LIBRARY_PATH PYTHONPATH="$PWD/src" uv run --no-sync pytest …`.** A bare
`uv run pytest` dies at `ImportError … GLIBC_2.43 not found` before collecting one test. The full
gate takes ~17 min, so run it in the background and work while it runs.

**Suite provenance.** The red suite lived at tag `archive/m2u83-test-4` = `ca2cb41` (2169 lines,
18 predicates, measured 58 failed / 2 passed) and was restored into the primary tree at window 2's
start. It must never be committed red: **`tests/test_r_timebase_truth.py::test_c8_08` runs the whole
suite in a subprocess and asserts rc=0**, so a red file anywhere in `tests/` takes the decisive gate
down. Wave record: `test-m2u83-2` P01-P04 (`archive/m2u83-test-2` = `aa4981e`), `test-m2u83-3`
P05-P13 (`archive/m2u83-test-3` = `2d1fba2`), `test-m2u83-4` P14-P18 (`archive/m2u83-test-4` =
`ca2cb41`). Report preserved at `.scratch/agents/test-m2u83-suite.md`.

#### M2.8.3 — DONE at `4c006bd`, 3 windows against a 2 estimate; split at the `FEATURES` seam

**Window 3 — `harvest=` n/a (no wave harvested), `main=` 87% 237K/273K, `mate=` 40% 108K/273K.**
Shipped: the corpus census, two contract rulings, and `src/pose_estimation/cohort.py` — the
publisher's contract-owned half. Did NOT ship: the aggregation engine, publication, the CLI, the
descriptor projection, docs, registration — **all of which are now M2.8.5.**

**The unit was SPLIT at the `FEATURES` seam by user ruling, and this record is why.** Three
consecutive overruns against a `2 est` were structural rather than estimative: M2.8.3 was two units
wearing one id. Its halves share no state — the schema half is a pure function of the R definitions
plus the contract, the compute half a pure function of the run — and everything the first produces
crosses to the second as one frozen tuple. A unit whose halves communicate through a single
immutable value should have been split at planning, and three windows is what the missed seam cost,
because each window re-paid the same entry cost (contract + corpus + state) before reaching new
work. **Sizing rule this yields: a seam that is one immutable value is a unit boundary, and PLANNING
should look for that shape before sizing.** M2.8.3 closes DONE at 3 spent; M2.8.5 carries the
heavier half at `2-3 est` and inherits the contract, the 22 amendments and the red suite.

**Window 3's substantive correction.** The roadmap predicted 92 published columns / 1104 feature
rows. Measured: **89 published / 3 excluded over 92 source columns, 1068 rows.** `--tracking body`
restored 14 of the 17 trunk/posture columns, not 17: a **second, mode-independent** guard at
`analysis/clinical_features.R:1038` assigns `NA_real_` to the sagittal-lean three on the 2D branch,
because sagittal lean is out of plane. **Datum: predicting a restored column set from one guard
being lifted overcounts by exactly the columns a second guard suppresses — count the guards, not
the columns.** A02's measured partition absorbed the correction with no predicate edit, which is
the payoff for having ruled the partition measured rather than declared.

Cross-check that did not move: frame/window rows stand at **331 152 / 21 483**, unchanged from the
pre-fix tree. Tracking mode and coordinate scale change values, not row counts, so this is an
independent check on the re-run rather than a restatement of it. Cells: 12/12 present, `n_subjects`
15-16 (floor 15 holds), 193 events, 379 assets all manifest-`ok`, **135 multi-asset events** = P06's
population bound. Lowest nonzero finite rates: `movement_efficiency` family 0.7569, `left_wrist_sal`
0.8860 — **P08's empty-column path is unreachable on real input and must be exercised
synthetically.** Census script: `.scratch/m2u83_census.py`, 5 s, aggregates only.

Everything this window did not ship is scoped, ordered and dispatchable under **M2.8.5** above.
...
`deg_image_plane` (the token as M2.8.4's isotropy fix left it; A09 ruled it
...
**One determination deferred to implementation, named in P10.** Coordinates were normalized to
...
corpus-wide, so the angle is a plain 2D image-plane angle, and the distortion was a measured median
**9.9°** (p95 26.5°). No published column carries unit `deg`; the 9 angle-derived columns ship
in a 7-token frozen vocabulary, assigned by a rule over source family × derivation suffix rather
than a transcribed table. **User ruling: keep the 9 columns with the qualified token rather than
dropping them.** **M2.8.4 then made the map a similarity** (one scalar `max(frame_w, frame_h)`), so
the shipped token is **`deg_image_plane`** and the 9.9° is history: the published angle now *is* the
image-plane angle, still uncorrected for lens distortion and still not anatomical.
...
- **A11 (F10)** — P12's witnesses were semantic validators, which canonicalize JSON and so cannot
  ...
  **M2.8.2 ruled the identical defect one unit earlier at `3225577`**, so the rule is promoted to
  `.claude/rules/evidence.md`: *a validation function is never a byte witness.*
...
and the diagnostics-reach limit live in `.claude/rules/corpus-run.md`, so they survive this
...
- **Detectability re-measured after a defect.** `detect_rate` median **1.0** (mean 0.989886, min 0.333333, n=379), 0 inference-failure frames, all four device configurations uniformly high. The prior 0.0 median was an artefact of rtmlib `PoseTracker` IoU-matching seconds-apart samples → `.claude/rules/rtmlib-runtime.md`.
