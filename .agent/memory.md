...
- **Standing local clauses + the reconciliation procedure live in `.claude/rules/drop-ins.md`** — project law that loads beside `CLAUDE.md` for MAIN and teammates alike, so a refresh dropping a clause from a command file no longer drops it from context. Durable law lands there; this file keeps the evidence behind it.
