...
- **`CLAUDE.md` + `.claude/commands/*.md` arrive as upstream drop-ins landing over this repo's local adaptations; the two standing clauses below are the only surviving copy.** Procedure, every refresh: diff each refreshed file against its prior commit, re-apply both clauses, keep every upstream change neither clause contradicts, commit `state: …`. Verify `rg -n 'DESCOPED|agent/contracts' .claude/commands/ CLAUDE.md` → the DESCOPED clause present, zero `agent/contracts` hits. `git log --grep upstream` lists priors. A refresh that retires a constant also falsifies claims elsewhere → sweep `.agent/` for the retired term and correct what depended on it, since a stale sizing datum reaches PLANNING as a budget. **Project law lives in this file, never in `.claude/rules/`** — upstream `CLAUDE.md` offers `.claude/rules/<topic>.md` as a scope and this project declines it (user ruling): `.agent/` is already refresh-proof because upstream never writes it, and refresh cadence is low enough that the law must be *reachable* when a refresh is reconciled, never *resident* every session. A `scripts/check_drop_ins.py` gate was weighed and declined on the same reasoning — the user announces every refresh, and both clauses already self-evidence against the tree: 15 files name the archive path, so a reverted command file contradicts them on sight, and M3's own roadmap heading is the second guard on DESCOPED. Rigor concentrates where no re-check exists; here two do.
- **Clause 1 — DESCOPED = a terminal milestone status** (`session-roadmap.md:3`); every refresh to date drops it. Active milestone = first lacking a terminal status (DONE / REVIEWED / DESCOPED). A DESCOPED milestone is closed by decision rather than by completion: its retained detail lives in the `.agent/archive/` record the roadmap stub names, and reviving any part of it is a PLANNING ruling, never a dispatch outcome. Damage is deferred rather than immediate — M2 is IN-PROGRESS, so the reverted text picks M2 either way, and the risk lands only once M2 reaches REVIEWED and M3 becomes "first awaiting DONE/REVIEWED". M3's own roadmap heading declares its terminality as the second guard.
...
- **Superseded by upstream — never restore.** `N% NK/<window>` replaces the local `N% NK/1M` gauge convention: MAIN's window is whatever `CLAUDE_CODE_AUTO_COMPACT_WINDOW` clamps it to and recorded figures already straddle 240K, 273K and 1M, so no literal belongs in it. "The harness manages the window" replaces "collapse-managed window sheds content" throughout — same mechanic, fixed terminology.
- **The `Read()` path-exclusion control is RETIRED — never restore it.** A refresh replaced the two-control read-exclusion set with `.gitignore` currency + Serena `ignored_paths`, and `.claude/settings.json` was deleted with it, because all 31 of its entries were `Read()` rules. `permissions.deny` is Bash-only now: a path-keyed `Read()` rule also gates `Bash` by static command text, so a batched command dies for naming the path it excludes, and one command whose cwd the matcher cannot resolve halts the session under `bypassPermissions`. `.agent/archive/m2u71.md` and `contract-m2u2.md` §6 still name that deny list; both are frozen unit records of a control that no longer exists, never a live surface.

## Publisher trust roots — the surface no digest covers

- **Every self-describing publisher has exactly one unverified file: its own marker.** `tree_digest` must exclude the marker because the marker carries that digest, so nothing inside the set covers it. Three properties therefore have to be checked directly, and `measure` had all three while `qualify` had none until M2.3 window 8 — sibling publishers drift, so check each one rather than assuming the idiom spread. (1) The marker is a regular non-symlink file (`lstat` + `S_ISREG`); a symlink puts the trust root outside the set, and where ownership gates a recursive delete it lets a foreign directory license its own deletion. (2) The parse rejects duplicate keys (`object_pairs_hook`); `json.loads` keeps the last silently, so one document carries two claims. (3) The census digest covers the provenance block minus its own self-referential key — excluding the whole block leaves the upstream claims consumers trust most as the only uncovered content in the set. The six publishers are `inventory`, `sessions`, `qualify`, `measure`, `calibration_qc`, `cohort`. **Measured at M2 review wave 1, and the answer is negative on both counts: `inventory.py:1075` and `sessions.py:812` `read_text` + `json.loads` their markers, so both follow a marker symlink and both accept a duplicate-key document** — the other four reject each. Property (3) also fails twice: `sessions.py:588` and `measure/__init__.py:281` drop the whole provenance block from the self-digest, so a synthetic edit to either marker's upstream claim validates without recomputing any digest. Idiom drift is the mechanism every time — a property lands where it was first ruled and does not travel to siblings, so **check each publisher rather than the family** (`.agent/review-m2.md` T5 R15/R16/R17, ACCEPT-FIX; fix = one shared `lstat`+`S_ISREG`+`object_pairs_hook` loader across all six).
- **A prefix-matching name is not an ownership test, and a sweep built on one deletes what it was written to protect.** `cohort` staged into `f"{out.name}.staging.{os.getpid()}"`, removed that path before publishing and recursively deleted every prefix-matching sibling after the swap. Both halves are wrong for the same reason: a pid is neither unique (reuse) nor private (any process can create the name), so the pre-run remove destroys the only complete staged generation under pid reuse and the post-swap sweep destroys a foreign directory that merely shares the prefix. **The fix is to make the name unguessable rather than to make the sweep smarter** — `tempfile.mkdtemp(prefix=…, dir=out.parent)` owns its name by construction, so `_sweep_orphans` was **deleted** from `cohort.py` and cleanup touches only this invocation's own paths (M2 review T3 R29/R30/R31, A30). Idiom drift again: **`sessions.py`, `calibration_qc.py` and `qualify.py` still carry the prefix-name sweep**, recorded on the declined publisher-extraction polish row as its fifth measured drift.
- **A set digest over file contents does not cover set membership.** `cohort.tree_digest` hashed each published file and accepted an arbitrary extra one, because nothing in the digest said which names the set contains. It now folds the sorted entry-name set — marker excluded, so the staging and published digests agree — into the digest, and a fifth file fails `validate_generation` (T3 R25, A26). Check every publisher's digest for the same gap: content coverage and membership coverage are separate properties.
...
- **MILESTONE-REVIEW wave sizing, measured on M2 wave 1: 5 reviewers × ~40 rows = 193 rows / 55 fails fits one MAIN window**, but only because harvest is mechanical — archive the report, script-extract the verdict table, read `fail` rows alone; a 193-row harvest then costs ~25K. Seed each reviewer's report skeleton (every target row, verdict `unknown`) and commit it before dispatch, and grade the validator both ways at seed so the seed itself proves the gate discriminates.
- **Run the decisive gate alone.** Five concurrent worktree gates plus MAIN's own drove `tests/test_r_timebase_truth.py::test_c8_08` past its 900 s `subprocess` timeout on 8 cores — the failure is CPU contention, not a defect, and it is the whole difference between `1701 passed / 1 failed` and a green suite. Brief reviewers to targeted files and keep the full-suite slot for MAIN.
...
- **The gate recipe and the corpus-run recipe are MUTUALLY EXCLUSIVE, and picking wrong is silent.** The standing gate prefix `env -u LD_LIBRARY_PATH PYTHONPATH="$PWD/src" uv run --no-sync` is exactly what strips the accelerator: it reports `['CPU']`, compiles, runs, and produces correct output — no warning anywhere, because CPU is a supported device. Only the wall clock tells. Measured on the same two events, `--tracking body`: **1015.5 s / 927.4 s under the gate prefix vs 638.5 s / 497.2 s** under the run recipe = **1.6-1.9x**, which is 8 h against 15 h over the corpus. A decode/inference run therefore uses the documented recipe INSTEAD, never the gate prefix — `source /var/home/eturkes/.local/app/intel-accel/env.sh` then `PYTHONPATH="$PWD/src:$PYTHONPATH" .venv/bin/python scripts/…` (the module docstring of `corpus_run_2d.py` carries it). Verify placement before funding hours, not after: `openvino.Core().available_devices` must read `['CPU', 'GPU', 'NPU']`, and the event's own `run.log` must open `pose-device=NPU` + "loaded … with the openvino/NPU backend".
...
- **2D features are per-camera quantities, so view is a large published variance component.** Over
  ...
  sound. Cause is projection geometry — coordinates are normalized to `[0,1]` by **one scalar,
  `max(frame_w, frame_h)`** (`export.coord_scale`), so each camera measures in its own image plane
  and no cross-camera map exists without extrinsics. Consequence:
...
- **The `[0,1]` normalization WAS anisotropic at 1920×1080 and cost a measured median 9.9° on
  every published angle; M2.8.4 fixed it and every live consumer must state the fixed form.** The
  retired code divided x by width and y by height; `z` is identically
...
## Review-report grader (`scripts/check_review_report.py`)

- Committed, so a MILESTONE-REVIEW wave's report-shape claim reruns from committed state. 9 predicates over the two-tier shape: three required sections, parseable `| <ID> |` rows, unique ids, a verdict cell per row, **zero `unknown`** (P03 — this is what makes an all-`unknown` seed grade nonzero), every `pass` row stating what was checked, a `### <id>` detail section per `fail` row carrying `file:line` + `predicate` + `impact` + `acceptance`, and an acceptance check on every register entry. Seed each reviewer's skeleton with all rows `unknown`, grade it both ways before dispatch, and name the command in the brief — the seed plus the validator is the pair that yields units; a prose mandate alone yields none.
