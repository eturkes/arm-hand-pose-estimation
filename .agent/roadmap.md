...
**MILESTONE-REVIEW wave 1 — done; M2 stays IMPLEMENTED because adjudicated ≠ fixed.** Five reviewers in one block off `384c15f` (`rev-m2u82` M2.8.1+M2.8.2 · `rev-m2u84` M2.8.4 · `rev-m2u85` M2.8.3+M2.8.5 · `rev-m2-cross` cross-unit + M2.7.3/M2.7.4 docs + CLAUDE.md conformance · `audit-m2` claim replay M2.7.3-M2.8.5) enumerated **193 check-set rows, 55 fail**, every one adjudicated in `.agent/review-m2.md`: **43 ACCEPT-FIX · 9 ACCEPT-LIMIT · 2 POLISH · 3 REJECT**, 0 open. Reports at `.agent/archive/review-m2-<name>.md`; reviewer tips tagged `archive/m2-review-<name>` (three carry committed red batteries — 490 / 34 / 226 insertions under `tests/`, red against shipped code). **The next MILESTONE-REVIEW session applies the 42 still-open ACCEPT-FIX rows**, each closing on its own acceptance check under MAIN's rerun; REVIEWED waits on that. One closed in-session (X01: `uv sync --frozen` restored the missing `pose-estimation-cohort` console script, `.venv/bin` now 11/11). Decisive gate at wave close, **run alone from committed state: 1702 passed, 24:27, rc=0** — the 1701/1 seen during the wave was five worktree gates plus MAIN's timing out `test_c8_08`'s 900 s subprocess on 8 cores. Highest-value finding, confirmed by five independent lenses: **the corrected isotropic corpus still publishes `deg_image_plane_uncalibrated` on 19 angle descriptors and `docs/technical/cohort.md` still claims anisotropic normalisation with 9.9° error** — a shipped clinical artifact carrying false metadata, so its roadmap and contract passages stay as-written until the republish closes the row. Wave sizing datum: 5 reviewers × ~40 rows fits one MAIN window when harvest reads the verdict table and opens `fail` rows alone.
