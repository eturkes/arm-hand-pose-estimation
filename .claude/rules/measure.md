...
- `drift_ppm`/`drift_se` carry no status column, so their emptiness (114/246 rows) never says which of `short_overlap`, `insufficient_windows`, `degenerate_regression` or `global_abstention` caused it. `measure.DRIFT_STATUSES` already names the four tokens; P36 froze the column list and nothing downstream consumes the reason (`.agent/archive/polish.md`).
- `SUPPORTED_VERSIONS` is one module-wide frozenset, so a schema change on one axis invalidates every axis — over-invalidation is a granularity defect, not a correctness one (`.agent/archive/polish.md`).
