...
- **Clause 3 — mutable state belongs in `.agent/spec.md`, never in `.claude/rules/`.** A rule
  file is read as standing law, so a mid-session ledger parked there reads as current long after
  it stops being true. Anything MAIN rewrites while it works — status, open rows, counts — stays
  in `spec.md`; `.claude/rules/` takes only what holds until new evidence reverses it.
- **Clause 4 — `.agent/archive/` is this repo's detail store and its records are frozen.**
  ...
  `.agent/spec.md` and `.claude/rules/`.
...
and the phase flow replaces the MODE dispatch. `.serena/project.yml` `ignored_paths` and the
`read-guard.sh` volume budget: both mechanisms are deregistered toolchain-wide.
