...
- **A deliverable-first seed leaves its suite wiring FAIL on purpose.** A case asserting `rc == 0` would be red until the artifact lands, and a skip guard breaks the zero-skip invariant `test_c8_08` reconciles. The test lands in the same commit as the artifact it grades; seeding a passing stub instead would encode the defect. Its red witness is therefore the pre-commit targeted run on the unfixed tree, never a red commit (→ `gates.md`).
