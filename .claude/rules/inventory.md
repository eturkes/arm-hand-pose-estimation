...
- **`check_inventory_determinism.py` binds to a spelled-out 4-entry `SOURCE_FILES`** — checker, `__init__.py`, `inventory.py`, `video_io.py` — records `source_sha256` per file, and exits 2 rather than overwrite a result measured against other bytes (`schema_version` 2; M2 review T5 R09). No head SHA: the regenerating run always precedes the commit carrying its output, so a recorded SHA names the parent state. Two cases in `tests/test_inventory_review.py` grade it — one recomputes every digest from the worktree, one drives the refusal on both arms. **`run_inventory_mutations.py:817` still records `tested_head`**; that drop rides the `.agent/spec.md` `Deferred` row, which already forces the regeneration a schema change needs.
