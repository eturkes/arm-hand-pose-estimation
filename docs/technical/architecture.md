...
| `cohort.py` | Publishes the `(task, side)` cohort aggregate from a corpus run. Carries the feature label table and measures the published column set from the run. See `cohort.md`. |
