...
| `tests/test_sessions.py` | Covers the 18 acceptance predicates of the session generator: placement conservation over every registry row, event-identity type safety and its two-digit grammar, take resolution for view-conflict families, discovery and `--list-sessions` agreement, byte-identical regeneration across locale, hash seed, time zone and output name, idempotency with stale removal, every generation tamper class, the closed marker schema, registry-first validation, synthetic escaped-path decoding, symlink extension and containment, console and manifest redaction, the two-rename publication swap, the instrumented no-corpus-listing proof, and marker-shaped output ownership. It also pins destructive-overlap refusal (an output equal to, containing, or inside either input), publication through a symlinked `--out`, and the dead-pid sweep of staging and retiring siblings, links included. A final group pins registry-shape refusal — missing column, repeated id, unknown disposition, orphan family, malformed `view_conflict`, mixed grammar version, and every cell alphabet — plus the link-time source recheck, rollback of a failed swap, and the published schema fields (`grammar_version` on placements, numeric manifest `subject_ordinal`). A last group pins the four checks that read as stricter than they are: a swap failing into an absent root leaves the dead run's retired generation intact and raises the real error; a sibling identifier wider than a C long sweeps as debris instead of aborting; a trailing newline is refused in every alphabet and in the exported `EVENT_ID_PATTERN`; a `subject_ordinal` outside the ASCII digits is refused in both the raising and the silently-normalizing form; and a short header is refused on a table with no rows, against a whole-header control that still publishes. |
| `tests/test_qualify.py` | Capture-qualification schemas, measurement ingestion, row derivation, alphabets, redaction, upstream freshness, ownership, atomic publication, and consumer validation. |
| `tests/test_m2u5_alignment.py` | M2.5 alignment sign, solver, reference, status partition, census, canonical ordering, documentation, and committed determinism evidence. |
| `tests/test_m2u5_mutants.py` | Focused oracle for the 25-mutant M2.5 alignment campaign. |
| `tests/test_calibration_qc.py` | Fixed corpus ruling, long-form evidence, probe-digest binding, claim conformance, redaction, ownership, atomic publication, CLI behavior, and consumer validation. It also pins every refusal path: the closed evidence-record key set, the five required grouping arms at the ruled population, duplicate arm labels, the required statistic fields, one-row corpus tables, capture-into-output overlap, and the derived prohibitions on unrun-arm outcomes. A final group pins order and spelling — every input is validated before any output is touched, each refusal carries a reason code, and the published headers are spelled out. |
| `tests/test_cohort.py` | The cohort publisher end to end: the four-stage asset -> event -> subject -> cohort estimand, the label table and its unit and range rules, the measured published-column partition and its cross-check against `FEATURES`, `view_dispersion` over multi-view events, the five-subject distribution floor, the descriptor projection and its collision check against `../rehab`, atomic publication with ownership and debris sweeping, the consumer digest boundary, the CLI, and the registration of every generated tree. |
| `tests/test_calibration_qc_mutants.py` | Focused oracle for the 51-mutant calibration-QC publisher campaign. |
| `scripts/check_qualify_determinism.py` | Qualification campaign: 40 sweeps across both publication modes, plus 19 consumer-boundary tamper classes. |
| `scripts/check_m2u5_determinism.py` | M2.5 campaign for canonical row order, output-name independence, and repeat publication. |
| `scripts/run_m2u5_mutations.py` | M2.5 campaign: 25 source mutants with baseline, exact restoration, and final-baseline checks. |
| `scripts/check_calibration_qc_determinism.py` | Calibration-QC campaign: 21 publication sweeps and 18 consumer-boundary tamper classes. |
| `scripts/run_calibration_qc_mutations.py` | Calibration-QC campaign: 51 publisher mutants with catalogue, baseline, restoration, and final-baseline checks. |
