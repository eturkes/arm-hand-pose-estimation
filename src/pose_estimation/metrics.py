...
class MetricsCollector:
    ...
    def record(
        ...
        # Jitter
        row["body_jitter_px"] = self._jitter(body_lm_smooth, self._prev_body_lm, person_idx)
        row["hand_L_jitter_px"] = self._jitter(hand_L_smooth, self._prev_hand_L, person_idx)
        row["hand_R_jitter_px"] = self._jitter(hand_R_smooth, self._prev_hand_R, person_idx)
        ...
        # Smoothing delta
        row["body_smooth_delta_px"] = round(smooth_diag.body_smooth_delta_px, 2)
        ...
        # Constraints
        row["bone_correction_px"] = round(constraint_diag.bone_correction_px, 2)
        ...
        # Matching
        row["hand_arm_match_dist_L"] = round(match_dist_L, 2) if match_dist_L is not None else ""
        row["hand_arm_match_dist_R"] = round(match_dist_R, 2) if match_dist_R is not None else ""
        ...
        if self._detail_w is not None:
            ...
            self._write_detail(
                ...
                person_idx,
                hand_L_smooth,
                frame_diag.raw_hand_landmarks[0] if frame_diag.raw_hand_landmarks else None,
                None,
                "hand_L",
            )
            self._write_detail(
                ...
                if len(frame_diag.raw_hand_landmarks) > 1
                else None,
                None,
                "hand_R",
            )
    ...
    def _write_detail(self, frame_idx, person_idx, smooth_lm, raw_lm, vis, part):
        ...
        prev_key = {
            "body": self._prev_body_lm,
            "hand_L": self._prev_hand_L,
            "hand_R": self._prev_hand_R,
        }[part]
        ...
        for kp in range(n_kp):
            ...
            if prev is not None and prev.shape[0] > kp:
                row["jitter_px"] = round(float(np.linalg.norm(smooth_lm[kp, :2] - prev[kp, :2])), 2)
